...
Status MLUPowerDifference::CreateMLUOp(std::vector<MLUTensor *> &inputs,
  ...
  TF_STATUS_CHECK(lib::CreatePowerDifferenceOp(&power_difference_op_ptr,
                                        final_input1, final_input2, power_c, output, len));
...
Status MLUPowerDifference::Compute(const std::vector<void *> &inputs,
  ...
  TF_STATUS_CHECK(lib::ComputePowerDifferenceOp(
                                 power_difference_op, queue,
                                 broadcast_1_addr, broadcast_2_addr, output));
